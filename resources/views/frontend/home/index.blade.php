...
@section('content')
            ...
            <div class="row">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 theme-default">
                    <div class="slider-events-list">
                        ...
                        <img src="{{asset($slider->slider_img)}}"  title="#{{$slider->id}}" />
                            @endforeach
                        @else
                            <img src="{{asset('public/frontend/demo/images/groupblood.jpg')}}" data-thumb="{{asset('public/frontend/demo/images/toystory.jpg')}}" alt="" title="#htmlcaption" />
                        @endif
                    </div>
                    @if(count($sliders)>0)
                        @foreach($sliders as $slider)
                    <div id="{{$slider->id}}" class="nivo-caption">
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-3 col-md-3 col-sm-12 col-xs-12 col-mb-12">
                    <h4 class="text-primary"><img src="{{asset($primaryInfo->logo)}}" width="15"> About Organization</h4>
                    <div class="text-justify well">
                        ...
                        <a title="{{$primaryInfo->company_name}}" href="{{URL::to('/about-organization/1')}}"><i class="fa fa-chevron-circle-down" aria-hidden="true" ></i> <span class="text-success">Read More</span></a>
                    </div>
 ...
 <hr>
            ...
            <div class="register-login">
                <div class="row">
                    <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                        <a href="{{URL::to('/member-register.html')}}">
                            <img src="{{asset('/images/banner/registration.png')}}" class="login-reg-img image-responsive center-block img-thumbnail">
                        </a>
                    </div>
                    <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                        <a href="{{URL::to('/member-register.html')}}">
                            <img src="{{asset('/images/banner/inspiration.png')}}" class="login-reg-img image-responsive center-block img-thumbnail"></a>
                    </div>
                    <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                        ...
                        @endif>
                            <img src="{{asset('/images/banner/login.png')}}" class="login-reg-img image-responsive center-block img-thumbnail"></a>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="container">
        @foreach($topMembers->chunk(5) as $members)
        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 col-mb-12 table-responsive top-ten-donors">
            <h3>Top blood donors list</h3>
                <table class="table table-bordered table-hover table-striped">
                    ...
                    <?php $i=1?>
                    @foreach($members as $topMember)
                        <tr>
                            <td><a href="javascript:void(0);" onclick="topTenDonorProfile({{$topMember->user_id}})">{{$topMember->name}}</a></td>
                            <td>
                                @if(!empty($topMember->photo))
                                <img src="{{asset($topMember->photo)}}" class="img-responsive" style="width: 50px; height: 50px;cursor: pointer" onclick="topTenDonorProfile({{$topMember->user_id}})">
                                    @else
                                    <img src="{{asset('images/default/photo.png')}}" class="img-responsive" style="width: 50px; height: 50px;cursor: pointer" onclick="topTenDonorProfile({{$topMember->user_id}})">
                                @endif
                            </td>
                        </tr>
                    @endforeach
    ...
    <script>
            ...
            function topTenDonorProfile(useId) {
                $("#top-donor-profile").modal('show');
                    $.ajax({
                    url:'{{url('/top-ten-donor-profile')}}'+'/'+useId ,
                    type: 'GET',
                    'dataType' : 'json',
                    success: function(data) {
                        console.log(data)
